...
#The first step is to create a model, which takes as arguments the number of
# constraints in your model, and the number of decision variables in your model.
...
# Now we need to set up the constraints and objective. The best way to do this
# using the lpSolveAPI package is by viewing the constraints in a matrix format.
...
# So the first column in our constraint matrix is c(1,1,0), and the second
# column in our constraint matrix is c(1,0,1). We also need to indicate that
# these are less-than-or-equal constraints, and set the right-hand-side values
# to c(166,100,150):
...
# The default setting is minimize for the objective, so we need to tell R that
# we are maximizing our objective:
...
# You should see that there is a row called "Lower" in the model output - the
# values of zero indicate that our decision variables are constrained to be
# non-negative, which is the default setting. If you want to change the lower
# or upper bounds for a decision variable, you can do so with the set.bounds
# function. (Next week we will learn about integer and binary decision variables
#- to change a decision variable to one of these types, you can use the 
# set.type function.)
...
# You should get an objective value of 77408 and decision variable values
# of 100 and 66, just like we did in lecture.
...
# You should get an objective function value of 120514 and decision variable
# values of 80, 0, 75, 11, 60, 26.
